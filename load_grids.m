...
bpslopeh   = fread(fids.bpslope,[nx,ny],'real*4');
c0h        = fread(fids.c0,[nx,ny],'real*4');
mcor_origh = fread(fids.mcor_orig,[nx,ny],'real*4');
mcor_resh  = fread(fids.mcor_res,[nx,ny],'real*4');

tphs_origh = fread(fids.tphs_orig,[nx,ny],'real*4');
tphs_resh  = fread(fids.tphs_res,[nx,ny],'real*4');

for i=1:nd-1
    permh(i,:,:)=fread(fids.perm(i),[nx,ny],'real*4');
end
for i=1:nd
    hph(i,:,:)=fread(fids.hp(i),[nx,ny],'real*4');
    for j=1:ks
        mkh(i,j,:,:)=fread(fids.mk(i,j),[nx,ny],'real*4');
    end
end
for j=1:ks
    tphs_reskh(j,:,:)  = fread(fids.tphs_resk(j),[nx,ny],'real*4');
    slopeh(j,:,:)     = fread(fids.slope(j),[nx,ny],'real*4');
end

%while we only have one k
tphs_reskh=squeeze(tphs_reskh);
slopeh=squeeze(slopeh);
mkh=squeeze(mkh);
